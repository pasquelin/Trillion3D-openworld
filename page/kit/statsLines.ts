...
interface StatsSample extends FrameCounters {
  fps: number | null;
  held: boolean;
 ...
 * so, when the frame does not measure them; a still image keeps its last rate, marked held, and
 * its last GPU time, marked last.
 */
function statLines(sample: StatsSample): [string, string][] {
...
function sceneTriangles(scene: SceneNode): number {
  let total = 0;
